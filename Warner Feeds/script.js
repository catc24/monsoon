...
function displayNotification(warning, notificationType = "") {
    ...
    } else if (eventName.includes("Severe Thunderstorm Warning (Destructive)")) {
    ...
    // Start animation to show notification (slightly slower)
    notification.style.transform = 'translateY(100%)'; // Start off-screen
    notification.style.transition = 'transform 0.75s cubic-bezier(0.4, 0, 0.2, 1)'; // Smooth sliding effect

    // Start animation to show notification
    setTimeout(() => {
        notification.style.transform = 'translateY(50%)'; // Slide-up to the screen
    },-5); // Reduced delay for faster appearance
...
document.getElementById('tacticalModeButton').addEventListener('click', async () => {
    console.log('Save button clicked - fetching ALL warnings regardless of SAME codes');
